...
export default class Board implements IBoard {
	...
	private setStart = (sprites: ISprite[]): void => {
		...
		const sprite = sprites.find(
			(spr: ISprite) => spr.key === `sprite-${this.startX}-${this.startY}`,
		);
	...
	private setBonus = (sprites: ISprite[]): void => {
		...
		const y = this.yVal();
		const sprite = sprites.find(
			(spr: ISprite) => spr.key === `sprite-${x}-${y}` && spr.type !== SpriteTypeEnum.START,
		);
	...
	private newBlock = (x: number, y: number): ISprite =>
		new Sprite({
			key: `sprite-${x}-${y}`,
			visable: true,
			x: x * 3 + this.X_OFFSET,
	...
	private randomDirection = (): DirectEnum =>
		this.directions[Math.floor(Math.random() * this.directions.length)];
	private randomImage = (): ImageEnum =>
